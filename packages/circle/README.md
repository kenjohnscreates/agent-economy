...
6. Treasury e2e (M1.6): `pnpm --filter @agent-town/circle treasury-e2e` (dry-run plan) → orchestrator `… treasury-e2e --yes`. Ada `registerAgent(bo, "bo.botanica.eth")` → bo deposit 0.2 USDC → requestLoan 0.3 / approve / repay → tiny 1s loan 0.2 USDC → wait grace (`gracePeriodSeconds`, 120s on the live deploy) → `markDefault`. Approve USDC before deposit/repay. SCA fee is `DEFAULT_FEE` (MEDIUM), never `absoluteFee`. Amounts 0.2–0.5 USDC so treasury liquidity remains. Idempotent.
...
- Re-running `treasury-e2e --yes` is idempotent (skip register/deposit/loans already done). Default is dry-run; refuses to broadcast without `--yes`. `markDefault` waits until `block.timestamp > dueAt + gracePeriodSeconds` (120s on the live TownTreasury).
