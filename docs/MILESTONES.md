...
- M9.6 **Visitor agent + UI** (#61 `37239f5`): custom ENS label, Circle SCA, treasury `registerAgent`, **Your agent** panel under the map, 9th `AgentCard` / 3×3 grid, allowlisted chat (balance + deposit). Map stays 8 sprites. Fund **Arc USDC**. **Merged.**
- M9.7 **Public mint + 30-day quote + subagents**: public visitor create/chat (`ALLOW_VISITOR`, mayor stays on `ALLOW_BROADCAST`), 1 name per browser, ENS subdomain subagents, deposit reply uses live `townRateBps` × 30d / 365d. Fund Arc USDC (gas = same asset). T1.
