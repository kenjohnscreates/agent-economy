...
| M0.8 | Foundry init + Arc/Sepolia RPC config; `forge build` passes with viem `arcTestnet` | T3 | BE | `forge test` runs | M0.5 |
...
| ID | Task | Tier | Exit criteria | Deps |
|---|---|---|---|---|
| M4.1 | Tick engine loop, idempotent action ledger (Supabase), feature flags | T2 | 5 dry ticks logged | M0.6 |
| M4.2 | Role rules (consumer, merchant, worker, treasurer) per PRD §5 on subgraph signals | T1 | unit tests with fixture states | M3.4 |
...
**Exit M4:** 20+ unattended ticks; txs on Arc; subgraph reflects; SSE stream live.
...
## Stretch (only if M6 exits early)
- M9.1 Mayor registers a new agent: mint subname + create Circle wallet from UI.
- M9.2 ERC‑8004 registration on Arc + ENSIP‑25 `agent-registration` record loop.
- M9.3 Arc mainnet deploy script dry‑run (Launch‑to‑Mainnet track).
